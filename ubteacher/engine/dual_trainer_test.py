...
from utils import FDA_source_to_target_test
...
class UBTeacherTrainer(DefaultTrainer):
    ...
    def run_step_full_semisup(self):
        ...
        label_data_k_tgt_style = label_data_k.copy()
        unlabel_data_k_src_style = unlabel_data_k.copy()


        # import pdb
        # pdb.set_trace()
        # if mean_img.shape[-1] < 2:
        batch_size=len(label_data_q)
        for i in range(0,batch_size):
            C, H, W = label_data_k[i]['image'].shape
            C, H2,W2 =unlabel_data_k[i]['image'].shape
            H0=max(H,H2)
            W0=max(W,W2)
            # src_image= label_data_k[i]['image'].resize((W0,H0),Image.BICUBIC).unsqueeze(0)
            src_image= label_data_k[i]['image'].unsqueeze(0)

            # tgt_image= unlabel_data_k[i]['image'].resize((W0,H0),Image.BICUBIC).unsqueeze(0)
            tgt_image= unlabel_data_k[i]['image'].unsqueeze(0)
        ...
        # pdb.set_trace()
            ...
            try:
                ...
                src_in_trg = FDA_source_to_target_test(src_image,tgt_image, L=0.1)  # src_lbl
            # trg_img = trg_in_trg.clone() - mean_img
                trg_in_src = FDA_source_to_target_test(tgt_image,src_image, L=0.1)
            except RuntimeError:
                pdb.set_trace()
            # pdb.set_trace()
            ...
            label_data_k_tgt_style[i]['image'] = fft_src_in_tgt
            unlabel_data_k_src_style[i]['image'] = fft_tgt_in_src
        ...
        if self.iter < self.cfg.SEMISUPNET.BURN_UP_STEP:
            ...
            label_data_q.extend(label_data_k_tgt_style)
            ...
            unlabel_data_k_src_style = self.remove_label(unlabel_data_k_src_style)
            ...
            # )
            unlabel_data_k_src_style = self.add_label(
                unlabel_data_k_src_style, joint_proposal_dict["proposals_pseudo_roih"]
            )

            all_label_data =  label_data_k + label_data_q
            all_unlabel_data = unlabel_data_k_src_style
